...
func (self *GateHandler1) OnClientStream(s gira.GatewayConn) {
...
func TestClientClose1(t *testing.T) {
	...
	errGroup.Go(func() error {
		go func() {
			gateway.Serve(&GateHandler1{})
		}()
		select {
		case <-errCtx.Done():
			time.Sleep(1 * time.Second)
			log.Info("aaaaaaaaaa", gateway.Stat.ActiveSessionCount)
			log.Info("aaaaaaaaaa", gateway.Stat.CumulativeSessionCount)
			gateway.Shutdown()
		}
		return nil
	})
...
func (self *GateHandler3) OnClientStream(s gira.GatewayConn) {
	var req gira.GatewayMessage
	var err error
	req, err = s.Recv(context.TODO())
	if err != nil {
...
func (self *GateHandler4) OnClientStream(s gira.GatewayConn) {
	var req gira.GatewayMessage
	var err error
	req, err = s.Recv(context.TODO())
	if err != nil {
