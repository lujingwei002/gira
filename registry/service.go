...
type service_registry struct {
	localPrefix   string // /local_service/<<AppFullName>><<ServiceUniqueName>>/			根据服务类型查找全部玩家
	servicePrefix string // /service/<<ServiceUniqueName>>/      					根据服务类型查找全部玩家
	services      sync.Map
...
func (self *service_registry) onServiceAdd(r *Registry, service *gira.Service) {
	log.Infow("service add", "service_name", service.UniqueName)
...
func (self *service_registry) onServiceDelete(r *Registry, service *gira.Service) {
	log.Infow("service delete", "service_name", service.UniqueName)
	for _, fw := range r.application.Frameworks() {
		if handler, ok := fw.(gira.ServiceWatchHandler); ok {
...
func (self *service_registry) onKvDelete(r *Registry, kv *mvccpb.KeyValue) error {
	...
	if lastValue, ok := self.services.Load(serviceName); ok {
		lastService := lastValue.(*gira.Service)
		log.Infow("service registry remove service", "service_name", serviceName, "peer", value, "last_peer", lastService.Peer.FullName)
		self.services.Delete(serviceName)
		self.onServiceDelete(r, lastService)
		if lastService.IsGrouped() {
		...
		log.Warnw("service registry remote service, but service not found", "service_name", serviceName, "peer", value)
	}
	return nil
}
...
func (self *service_registry) watchServices(r *Registry) error {
	...
	r.application.Go(func() error {
		...
		log.Infow("service registry started", "local_prefix", self.localPrefix, "watch_start_revision", watchStartRevision)
...
func (self *service_registry) unregisterSelf(r *Registry) error {
	...
	self.localServices.Range(func(serviceName any, v any) bool {
		...
		if txnResp.Succeeded {
			log.Infow("service registry unregister self", "service_name", serviceName)
		} else {
			log.Warnw("service registry unregister self", "service_name", serviceName)
		}
		return true
	})
	return nil
...
// 注册服务
func (self *service_registry) RegisterService(r *Registry, serviceName string) (*gira.Peer, error) {
	client := r.client
	serviceKey := fmt.Sprintf("%s%s", self.servicePrefix, serviceName)
	localKey := fmt.Sprintf("%s%s", self.localPrefix, serviceName)
	kv := clientv3.NewKV(client)
...
// 查找服务位置
func (self *service_registry) WhereIsService(r *Registry, serviceName string) (peers []*gira.Peer, err error) {
	pats := strings.Split(serviceName, "/")
	if len(pats) != 1 && len(pats) != 2 {
		err = gira.ErrInvalidArgs.Trace()
		return
	}
	if len(pats) <= 1 || (len(pats) == 2 && pats[1] == "") {
