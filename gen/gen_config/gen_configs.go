...
func genConfigs1(configState *config_state) error {
	if data, err := ioutil.ReadFile(configState.configsFilePath); err != nil {
		return err
	} else {
	...
	for _, v := range configState.configsFile.descriptorDict {
		filePath := path.Join(configState.configDocDir, v.filePath)
		...
		configName := strings.Replace(filePath, configState.configDocDir, "", -1)
...
func genConfigs2(configState *config_state) error {
	log.Info("生成yaml文件")
	if err := os.RemoveAll(configState.configDir); err != nil {
		return err
	}
	if err := os.Mkdir(configState.configDir, 0755); err != nil {
		return err
	}
	for name, v := range configState.configsFile.descriptorDict {
		log.Info(name, "==>", v.yamlFilePath)
		filePath := path.Join(proj.Config.ConfigDir, v.yamlFilePath)
	...
	srcConfigsDir := path.Join(proj.Config.SrcGenDir, "configs")
...
// 生成协议
func Gen() error {
	log.Info("===============gen config start===============")
	// 初始化
	configState := &config_state{
		configDataDict:  make(map[string]*config_data),
		configsFilePath: path.Join(proj.Config.DocDir, "configs.yaml"),
		docDir:          proj.Config.DocDir,
		configDir:       proj.Config.ConfigDir,
		configDocDir:    filepath.Join(proj.Config.DocDir, "configs"),
		configsFile: configs_file{
	...
	if err := genConfigs1(configState); err != nil {
		log.Info(err)
		return err
	}
	if err := genConfigs2(configState); err != nil {
		log.Info(err)
		return err
	}
	log.Info("===============gen config finished===============")
	return nil
}
